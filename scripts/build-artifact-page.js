...
const output = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Grind X Coach</title><style>html,body,iframe{margin:0;width:100%;height:100%;border:0;background:#0b0c0d}iframe{display:block}</style></head><body><iframe id="view" title="Grind X Coach"></iframe><script>const pages=${payload};const view=document.getElementById('view');function show(name,anchor){if(!pages[name])return;view.srcdoc=pages[name];view.onload=()=>{const doc=view.contentDocument;doc.addEventListener('click',event=>{const link=event.target.closest('a');if(!link)return;const href=link.getAttribute('href')||'';const parts=href.split('#');if(pages[parts[0]]){event.preventDefault();show(parts[0],parts[1]);}});if(anchor)doc.getElementById(anchor)?.scrollIntoView();};}show('index.html');<\/script></body></html>`;
