...
```java
// Grind X: finalized callbacks must exclude speculative UNSTABLE_TEXT.
...
Upstream's helper appends the optional Android `UNSTABLE_TEXT` field. Grind X accepts only the finalized `RESULTS_RECOGNITION` list from the final callback. Interim callbacks are not used to create observations. No iOS dependency source is changed.
